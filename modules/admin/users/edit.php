...
if (isset($_POST['ok'], $_POST['name'], $_POST['email'], $_POST['pass'], $_POST['skype'], $_POST['about'])) {
    ...
    if(!count($error))	{
        $res_email = q("
   			SELECT `id`
   			FROM   `fw_users`
   			WHERE  `login` = '".es($_POST['email'])."'
   			AND    `login` <> '".es($_POST['is_login'])."'
   			LIMIT 1
   		");
            ...
            if ($test_preview == 'ok') {
                if (isset($_POST['img'])) {
                    foreach ($_POST['img'] as $img_key => $img_val) {
                        if (array_key_exists('preview', $_POST['img'])) {
                            $del_preview = q("SELECT `img` FROM `fw_users` WHERE `id` = " . (int)$_GET['id'] . "");
...
$res = q("SELECT * FROM `fw_users` WHERE `id` = ".(int)$_GET['id']."");
if(!$res->num_rows) {
