...
/// ##BASE NAVIGATION##
$input_modules = q("SELECT * FROM `modules` WHERE `parent` = 0 LIMIT 10");
if($input_modules->num_rows > 4) {
    $modules = [];
    $modules2 = [];

    $input_modules = q("SELECT * FROM `modules` WHERE `parent` = 0 LIMIT 0,4");
    while ($set_input_modules = $input_modules->fetch_assoc()) {
        $children_data = [];
        $child = q("SELECT * FROM `modules` WHERE `parent` = ".(int)$set_input_modules['id']."");
        if($child->num_rows) {
            while ($set_child = $child->fetch_assoc()) {
                $children_data[] = [
                    'id'   => $set_child['id'],
                    'name' => $set_child['name'],
                    'href' => $set_child['href']
        ...
        $modules[] = [
            'id'       => $set_input_modules['id'],
            'name'     => $set_input_modules['name'],
            'href'     => $set_input_modules['href'],
            'child'    => $children_data
        ];
    }

    $input_modules2 = q("SELECT * FROM `modules` WHERE `parent` = 0 LIMIT 4, 9");
    while ($set_input_modules2 = $input_modules2->fetch_assoc()) {
        $children_data2 = [];
        $child2 = q("SELECT * FROM `modules` WHERE `parent` = ".(int)$set_input_modules2['id']."");
        if($child2->num_rows) {
            while ($set_child2 = $child2->fetch_assoc()) {
                $children_data2[] = [
                    'id'   => $set_child2['id'],
                    'name' => $set_child2['name'],
                    'href' => $set_child2['href']
                ];
            }
        } else {
            $children_data2[] = [];
        ...
        $modules2[] = [
            'id'       => $set_input_modules2['id'],
            'name'     => $set_input_modules2['name'],
            'href'     => $set_input_modules2['href'],
            'child'    => $children_data2
        ];
    }
} else {
    $modules = [];
